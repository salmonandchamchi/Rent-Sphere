...
import com.niit.lease.model.entity.*;
import com.niit.lease.model.enums.ItemType;
import com.niit.lease.web.admin.mapper.*;
import com.niit.lease.web.admin.service.*;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.niit.lease.web.admin.vo.attr.AttrValueVo;
import com.niit.lease.web.admin.vo.graph.GraphVo;
import com.niit.lease.web.admin.vo.room.RoomDetailVo;
import com.niit.lease.web.admin.vo.room.RoomItemVo;
import com.niit.lease.web.admin.vo.room.RoomQueryVo;
import com.niit.lease.web.admin.vo.room.RoomSubmitVo;
...
public class RoomInfoServiceImpl extends ServiceImpl<RoomInfoMapper, RoomInfo> implements RoomInfoService {
    ...
    @Autowired private RoomAttrValueService roomAttrValueService;
    @Autowired private RoomFacilityService roomFacilityService;
    @Autowired private RoomLabelService roomLabelService;
    @Autowired private RoomPaymentTypeService roomPaymentTypeService;
    @Autowired private RoomLeaseTermService roomLeaseTermService;
    ...
    @Autowired private AttrValueMapper attrValueMapper;
    @Autowired private FacilityInfoMapper facilityInfoMapper;
    @Autowired private LabelInfoMapper labelInfoMapper;
    ...
    public void saveOrUpdateRoom(RoomSubmitVo roomSubmitVo) {
        ...
        if(isUpdate){
            ...
            LambdaQueryWrapper<RoomAttrValue> attrQueryWrapper = Wrappers.lambdaQuery(RoomAttrValue.class)
                    .eq(RoomAttrValue::getRoomId, roomSubmitVo.getId());
            roomAttrValueService.remove(attrQueryWrapper);
            //3.删除原有的roomFacilityList
            LambdaQueryWrapper<RoomFacility> facilityQueryWrapper = Wrappers.lambdaQuery(RoomFacility.class)
                    .eq(RoomFacility::getRoomId, roomSubmitVo.getId());
            roomFacilityService.remove(facilityQueryWrapper);
            //4.删除原有的roomLabelList
            LambdaQueryWrapper<RoomLabel> labelQueryWrapper = Wrappers.lambdaQuery(RoomLabel.class)
                    .eq(RoomLabel::getRoomId, roomSubmitVo.getId());
            roomLabelService.remove(labelQueryWrapper);
            //5.删除原有的paymentTypeList
            LambdaQueryWrapper<RoomPaymentType> paymentQueryWrapper = Wrappers.lambdaQuery(RoomPaymentType.class)
                    .eq(RoomPaymentType::getRoomId, roomSubmitVo.getId());
            roomPaymentTypeService.remove(paymentQueryWrapper);
            //6.删除原有的leaseTermList
            LambdaQueryWrapper<RoomLeaseTerm> termQueryWrapper = Wrappers.lambdaQuery(RoomLeaseTerm.class)
